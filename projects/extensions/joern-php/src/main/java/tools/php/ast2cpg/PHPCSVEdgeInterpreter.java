...
public class PHPCSVEdgeInterpreter implements CSVRowInterpreter
{
		...
		throws InvalidCSVFile
	{
		...
		if(row.getFieldForKey(PHPCSVEdgeTypes.TYPE).equals(PHPCSVEdgeTypes.TYPE_AST_PARENT_OF)){
			
			//a child note has one parent node
			//System.err.println(row.getFieldForKey(PHPCSVEdgeTypes.TYPE));
			if(!child2parent.containsKey(endId)) {
				child2parent.put(endId, startId);
			}
			else {
				System.err.println("Multiple parents nodes for child node:"+endId);
			}
			
			//a parent node may have multiple child nodes
			if(parent2child.containsKey(startId)) {
				//each childnum corresponding to one child node.
				if (!parent2child.get(startId).containsKey(childnum)) {
					parent2child.get(startId).put(childnum, endId);
				}
				else {
					System.err.println("childnum: "+childnum+" collide for parent node "+startId);
				}
			}
			else {
				parent2child.put(startId, new HashMap<Integer, Long>(){/**
					 * 
					 */
	...
	private int handleIncludeOrEval( IncludeOrEvalExpression startNode, ASTNode endNode, int childnum)
	{
		...
		switch (childnum)
		{
			case 0: // expr child
				...
				if (!startNode.getFlags().equals("EXEC_EVAL")) {	
					toTopLevelFile.includeLoc.add(startNode.getNodeId());
				}
	...
	private int handleGlobal( GlobalStatement startNode, ASTNode endNode, int childnum)
	{
		...
		switch (childnum)
		{
			case 0: // var child
				...
				if(startNode.getVariable().getNameExpression() instanceof StringExpression) {
					String globalName = startNode.getVariable().getNameExpression().getEscapedCodeStr();
					PHPCGFactory.globalMap.add(globalName, null);
				}
				break;
		...
		switch (childnum)
		{
			case 0: // expr child: Expression or null node
				if( endNode instanceof NullNode)
					...
					if(!(endNode instanceof NullNode))
						PHPCGFactory.ret.put(startNode.getFuncId(), endNode.getNodeId());
				}
				break;
	...
	private int handleAssign( AssignmentExpression startNode, ASTNode endNode, int childnum)
	{
		...
		switch (childnum)
		{
			case 0: // var child: Expression node
				...
				if(endNode instanceof Variable && ((Variable)endNode).getNameExpression() instanceof StringExpression) {
					String assignedVar = ((Variable)endNode).getNameExpression().getEscapedCodeStr();
					//we think it's a global variable
					if(PHPCGFactory.globalMap.containsKey(assignedVar)) {
						PHPCGFactory.globalMap.get(assignedVar).add(startNode.getNodeId());
					}
				}
				break;
	...
	private int handleUseElement( UseElement startNode, ASTNode endNode, int childnum)
	{
		...
		switch (childnum)
		{
			case 0: // name child: StringExpression node
				...
				collectUse.put(startNode.getNodeId(), ((StringExpression)endNode).getEscapedCodeStr());
				break;
	...
	private int handleUseStatement( UseStatement startNode, ASTNode endNode, int childnum)
	{
		...
		//topLevelFile => location of use ... as ...
		toTopLevelFile.useLoc.add(startNode.getNodeId());
		return 0;
	}
